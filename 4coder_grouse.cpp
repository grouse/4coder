    ...
    Project_File_Pattern_Array blacklist)
{
    ...
    for (u32 i = 0; i < list.count; i++, info++)
    {
        if (info->folder) {
            ...
            void *str_mem = heap_allocate(&global_heap, dir_size + info->filename_len);
            if (str_mem == nullptr) {
                i32 chunk_size = max(HEAP_CHUNK_SIZE, dir_size + info->filename_len);
                heap_extend(&global_heap, memory_allocate(app, chunk_size), chunk_size);
                str_mem = heap_allocate(&global_heap, dir_size + info->filename_len);
            }
            ...
            if (g_project_files_count == g_project_files_cap) {
                ...
                void *new_mem = heap_allocate(&global_heap, new_capacity * sizeof *g_project_files);

                if (new_mem == nullptr) {
                    i32 chunk_size = max(HEAP_CHUNK_SIZE, new_capacity * sizeof *g_project_files);
                    heap_extend(&global_heap, memory_allocate(app, chunk_size), chunk_size);
                    new_mem = heap_allocate(&global_heap, new_capacity * sizeof *g_project_files);
                }
...
GET_BINDING_DATA(get_bindings)
{
    ...
    begin_map(ctx, mapid_global);
    {
        ...
        bind(ctx, '/', MDFR_NONE, MODAL_CMD(search));
